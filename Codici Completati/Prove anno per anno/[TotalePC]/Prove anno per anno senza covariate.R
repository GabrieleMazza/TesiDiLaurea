...
for (year in 1997:2010)
{
    ...
    #Per lambda=10^3
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(377,434))
    #Per lambda=10^2
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(374,442))
    #Per lambda=10^1
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(361,520))
    #Per lambda=10^0
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(330,728))
    #Per lambda=10^-1
    plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(276,1252))
    dev.off()
}
