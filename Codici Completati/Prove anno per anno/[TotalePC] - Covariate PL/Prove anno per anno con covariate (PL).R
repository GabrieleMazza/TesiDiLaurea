...
for (year in 1997:2010)
{
    ...
    #Per lambda=10^3
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(364,416))
    #Per lambda=10^2
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(354,427))
    #Per lambda=10^1
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(287,494))
    #Per lambda=10^0
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(-165,633))
    #Per lambda=10^-1
    plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(-1453,744))
    dev.off()
}
