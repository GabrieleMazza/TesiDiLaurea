...
for(j in 1:length(TimePoints))
{
    ...
    ggplot(data=Knot, aes(x=xknot, y=yknot)) +
        ...
        guides(size=FALSE)
    ggsave(file=paste("Dati Generati al tempo ",round(TimePoints[j],3),".png",sep=""))
}
...
# Esegui i plot per confrontare i plot ad un dato tempo
TimeSelected=0
NXplot=300
NYplot=100
...
for(i in 1:NXplot)
{
    for(j in 1:NYplot)
    {
        if(!is.na(fs.test(xGen[i],yGen[j])))
        {
            ...
            RealValuesPlot<-c(RealValuesPlot,fun(xGen[i],yGen[j],TimeSelected))
        }
    }
}
...
tgrd <- as.POSIXct("2010-08-05")+3600*c(TimeSelected,1)
pred.grid_C <- STF(sp=grd, time=tgrd)
...
# Risolvo
SolutionObj<-ST.Smooth(Data,SpaceBasisObj,TimeBasisObj,LambdaS,LambdaT)
PredictionST<-ST.Eval(xPlot,yPlot,tPlot,SolutionObj)
...
Max<-max(PredictionKRIG,PredictionSOAP,PredictionST,PredictionTPS,RealValuesPlot)
Min<-max(PredictionKRIG,PredictionSOAP,PredictionST,PredictionTPS,RealValuesPlot)
...
{
    PlotMatrix[PosMatrix[i,1],PosMatrix[i,2]]=PredictionTPS[i]
}
# Plot
png(filename=paste("TPS tempo ",TimeSelected,".png",sep=""))
image(xGen,c(yGen,2),PlotMatrix,col=heat.colors(100),main=paste("Funzione reale tempo ",TimeSelected," TPS",sep=""),xlab="",ylab="",ylim=c(-1,1))
...
for(i in 1:(dim(PosMatrix)[1]))
{
    PlotMatrix[PosMatrix[i,1],PosMatrix[i,2]]=PredictionSOAP[i]
}
# Plot
png(filename=paste("SOAP tempo ",TimeSelected,".png",sep=""))
image(xGen,c(yGen,2),PlotMatrix,col=heat.colors(100),main=paste("Funzione reale tempo ",TimeSelected," SOAP",sep=""),xlab="",ylab="",ylim=c(-1,1))
lines(xbound,ybound,lwd=3)
lines(c(xbound[1],xbound[length(xbound)]),c(ybound[1],ybound[length(ybound)]),lwd=3)
contour(xGen,c(yGen,2),PlotMatrix,nlevels=10,add=TRUE,lwd=2,labcex=1.1)
dev.off()
...
# Plot
png(filename=paste("ST tempo ",TimeSelected,".png",sep=""))
image(xGen,c(yGen,2),PlotMatrix,col=heat.colors(100),main=paste("Funzione reale tempo ",TimeSelected," ST",sep=""),xlab="",ylab="",ylim=c(-1,1))
lines(xbound,ybound,lwd=3)
lines(c(xbound[1],xbound[length(xbound)]),c(ybound[1],ybound[length(ybound)]),lwd=3)
contour(xGen,c(yGen,2),PlotMatrix,nlevels=10,add=TRUE,lwd=2,labcex=1.1)
dev.off()
...
# Plot
png(filename=paste("REALE tempo ",TimeSelected,".png",sep=""))
image(xGen,c(yGen,2),PlotMatrix,col=heat.colors(100),main=paste("Funzione reale tempo ",TimeSelected," REALE",sep=""),xlab="",ylab="",ylim=c(-1,1))
lines(xbound,ybound,lwd=3)
lines(c(xbound[1],xbound[length(xbound)]),c(ybound[1],ybound[length(ybound)]),lwd=3)
contour(xGen,c(yGen,2),PlotMatrix,nlevels=10,add=TRUE,lwd=2,labcex=1.1)
dev.off()
...
for(i in 1:(dim(PosMatrix)[1]))
{
    ...
    PlotMatrix[PosMatrix[i,1],PosMatrix[i,2]]=PredictionKRIG[ind]
}
# Plot
png(filename=paste("KRIG tempo ",TimeSelected,".png",sep=""))
image(xGen,c(yGen,2),PlotMatrix,col=heat.colors(100),main=paste("Funzione reale tempo ",TimeSelected," KRIG",sep=""),xlab="",ylab="",ylim=c(-1,1))
lines(xbound,ybound,lwd=3)
lines(c(xbound[1],xbound[length(xbound)]),c(ybound[1],ybound[length(ybound)]),lwd=3)
contour(xGen,c(yGen,2),PlotMatrix,nlevels=10,add=TRUE,lwd=2,labcex=1.1)
dev.off()
