...
for(jj in 1:length(TimePoints))
{
    ...
    CovarValid<-rep(0,length(xPlot))
    NewPoints<-data.frame(xData=xPlot,yData=yPlot,TimeData=tPlot,DesMat=CovarValid)
...
Max<-max(DataMatrix,PredictionTPSVect,PredictionSOAPVect,PredictionSTSRVect,RealValuesVect,na.rm=FALSE)
Min<-min(DataMatrix,PredictionTPSVect,PredictionSOAPVect,PredictionSTSRVect,RealValuesVect,na.rm=FALSE)
## PLOT ##
...
for(jj in 1:length(TimePoints))
{
    ...
    image(xGen,c(yGen,2),PlotMatrix,col=heat.colors(100),main=paste("Funzione stimata al tempo ",round(TimeSelected,3)," (STSR)",sep=""),xlab="",ylab="",ylim=c(-1,1))
    lines(xbound,ybound,lwd=3)
    lines(c(xbound[1],xbound[length(xbound)]),c(ybound[1],ybound[length(ybound)]),lwd=3)
    contour(xGen,c(yGen,2),PlotMatrix,nlevels=10,add=TRUE,lwd=2,labcex=1.1)
    dev.off()
