...
for(ntimes in 1:NTIMES)
{
    ...
    IND<-(pnt.in.poly(cbind(x,y),Bound)$pip==1) & (ind <- inSide(fsb,x=x,y=y))
    x<-x[IND]
    y<-y[IND]
    t<-t[IND]
    ...
    nval=length(PredictionGAM)
    
    # Studio degli errori
    ErrorGAM<-c(ErrorGAM,sum((PredictionGAM-RealValues)^2)/nval)
    ErrorST<-c(ErrorST,sum((PredictionST-RealValues)^2)/nval)
    
}

png(filename="Confronto tra i metodi.png")
plot(ErrorGAM,ylim=c(min(ErrorGAM,ErrorST),max(ErrorGAM,ErrorST)),main="Confronto tra metodi in punti casuali",type='l')
