...
{  
    ...
    for(i in 1:length(LogS))
    {
        ...
        for(j in 1:length(LogT))
        {
            ...
            GCVMatrix[i,j]=(n/((n-EDFMatrix[i,j])^2))*t((z-zHat))%*%(z-zHat)
...
{  
    ...
    TempDM<-solve(t(DesMat)%*%DesMat)%*%t(DesMat)
    Temp1<-t(Pi)%*%DesMat%*%TempDM%*%Pi
    Temp2<-t(Pi)%*%solve(diag(dim(DesMat)[1])-DesMat%*%TempDM)
