...
for (yearindex in 1:length(Years))
{
    ...
    for (i in 1:length(Codici))
    {
        if(!is.na(Codici[i]))
        {
            ANS<-c(ANS,Risposta$DifferenziataPC[Risposta$Codice==Codici[i] & Risposta$Anno==year])
        }
    }
    #Scelgo la covariata
    if(year%%100<10)
    ...
    if(DoKriging)
    {
        ...
        image(X,Y,evalmatkrig,col=heat.colors(100), xlab="", zlim=Klimits,ylab="",asp=1,main=paste("Kriging ",Years[yearindex],sep=""))
