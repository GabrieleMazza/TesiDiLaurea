...
for (year in 1997:2010)
{
    ...
    for(i in 1:length(CodiciYear))
    {
        ...
        ALBPL<-c(ALBPL,get(nameALBPL)[Coord$Codice==CodiciYear[i]])/Pop$Popolazione[Pop$Codice==CodiciYear[i] & Pop$Anno==year]
        CMPPL<-c(CMPPL,get(nameCMPPL)[Coord$Codice==CodiciYear[i]])/Pop$Popolazione[Pop$Codice==CodiciYear[i] & Pop$Anno==year]
    }
    detach(Covar)
