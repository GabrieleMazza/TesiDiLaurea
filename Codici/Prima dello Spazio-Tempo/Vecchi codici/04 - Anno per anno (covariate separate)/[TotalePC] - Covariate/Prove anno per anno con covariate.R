...
for (year in 1997:2010)
{
    ...
    for(i in 1:length(CodiciYear))
    {
        ...
        ALB<-c(ALB,get(nameALB)[Coord$Codice==CodiciYear[i]])
        CMP<-c(CMP,get(nameCMP)[Coord$Codice==CodiciYear[i]])
    }
    detach(Covar)
    ...
    betahat= ( solve( t(X) %*% X ) ) %*% t(X) %*% (Z[,2]-fhat[1:length(ComuniLat),])
    print(betahat)
    ...
    #Per lambda=10^2
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(345,427))
    #Per lambda=10^1
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(269,487))
    #Per lambda=10^0
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(-83,623))
    #Per lambda=10^-1
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(-507,1051))
    #Per lambda=10^-2
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(369,487))
    #dev.off()
}
