...
for (year in 1997:2010)
{
    ...
    #Risposta
    Z = matrix(0,nrow=length(CMP),ncol=2)
    Z[,1] = 1:length(CMP)[1]
    Z[,2] = CMP
    #Applico il modello
    lambda = 10^(2)
    ...
    #Per lambda=10^3
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(3,98))
    #Per lambda=10^2
    plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(2,197))
    #Per lambda=10^1
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(2,43))
    #Per lambda=10^0
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(0,200))
    #Per lambda=10^-1
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(0,359))
    dev.off()
}
max(M)
min(m)
