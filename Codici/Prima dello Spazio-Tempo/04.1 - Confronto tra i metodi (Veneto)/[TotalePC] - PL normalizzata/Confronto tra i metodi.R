...
for (yearindex in 1:length(Years))
{
    ...
    for(i in 1:length(ANS))
    {
        ...
        SRsmooth = smooth.FEM.fd.CovarCI(Znew,DesignMatrixnew,fdobj,SRLambda,0.95)
        
        betahat<-SRsmooth$approxCIbeta[1,2]
        SRz<-eval.FEM.fd(x[i],y[i],SRsmooth$felsplobj)+betahat*PL[i]
        ...
        xK<-x[1:length(ANS)]
        yK<-y[1:length(ANS)]
        KRIGsmooth=Krig(x=cbind(xK[-i],yK[-i]),Y=ANS[-i],cov.function="stationary.taper.cov",Z=DesignMatrixnew,m=2)
        ...
        TPSsmooth=Tps(x=cbind(xK[-i],yK[-i]),Y=ANS[-i],Z=DesignMatrixnew,lambda=TPSLambda)
