...
for (year in 1997:2010)
{
    ...
    #Risposta
    Z = matrix(0,nrow=length(ALB),ncol=2)
    Z[,1] = 1:length(ALB)[1]
    Z[,2] = ALB
    #Applico il modello
    lambda = 10^(-1)
    ...
    #Per lambda=10^3
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(5,7))
    #Per lambda=10^2
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(4,11))
    #Per lambda=10^1
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(2,43))
    #Per lambda=10^0
    #plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(0,200))
    #Per lambda=10^-1
    plot.FEM.2D(Rifiuti$felsplobj,zlimits=c(0,360))
    dev.off()
}
max(M)
min(m)
