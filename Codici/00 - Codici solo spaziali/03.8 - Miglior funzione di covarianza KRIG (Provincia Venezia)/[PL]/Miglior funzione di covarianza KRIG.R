...
for (year in Years)
{
    ...
    for(i in 1:length(ANS))
    {
        ...
        KRIGsmooth=Krig(x=cbind(xK[-i],yK[-i]),cov.function="Exp.cov",Y=ANS[-i],m=2)
        KRIGz<-predict(KRIGsmooth,t(c(x[i],y[i])))
        Error1<-c(Error1,(KRIGz-ANS[i])^2)
        
        KRIGsmooth=Krig(x=cbind(xK[-i],yK[-i]),cov.function="Exp.simple.cov",Y=ANS[-i],m=2)
        KRIGz<-predict(KRIGsmooth,t(c(x[i],y[i])))
        Error2<-c(Error2,(KRIGz-ANS[i])^2)
        
        KRIGsmooth=Krig(x=cbind(xK[-i],yK[-i]),cov.function="stationary.cov",Y=ANS[-i],m=2)
        KRIGz<-predict(KRIGsmooth,t(c(x[i],y[i])))
        Error3<-c(Error3,(KRIGz-ANS[i])^2)
        
        KRIGsmooth=Krig(x=cbind(xK[-i],yK[-i]),cov.function="stationary.taper.cov",Y=ANS[-i],m=2)
        KRIGz<-predict(KRIGsmooth,t(c(x[i],y[i])))
        Error4<-c(Error4,(KRIGz-ANS[i])^2)

        KRIGsmooth=Krig(x=cbind(xK[-i],yK[-i]),cov.function="wendland.cov",Y=ANS[-i],m=2)
        KRIGz<-predict(KRIGsmooth,t(c(x[i],y[i])))
