...
Create.Bspline.Time.Basis = function(TimePoints, TimeOrder,PlotIt=FALSE, NumBasis=NA)
{
...
smooth.ST.fd = function(Data,SpaceBasisObj,TimeBasisObj,LambdaS,LambdaT,DerivativeOrder)
{
    ...
    # Create S matrix
    t1<-proc.time()
    S=MakeS(SpaceBasisObj,TimeBasisObj,LambdaS,LambdaT,2)
    t2<-proc.time()
    DeltaT<-t2-t1
    print("S Matrix: ")
    print(DeltaT)
...
MakePi = function(SpaceBasisObj,TimeBasisObj)
{
    ...
    Phi<-sparseMatrix(i=row,j=col,x=val)    
    ...
    Psi<-sparseMatrix(i=row,j=col,x=val)  
...
MakeS = function(SpaceBasisObj,TimeBasisObj,LambdaS,LambdaT,DerivativeOrder=1)
{
    ...
    Sspace<-sparseMatrix(i=row,j=col,x=val)
    ...
    Stime<-sparseMatrix(i=row,j=col,x=val)  
    
    ### S=SSPACE+STIME ###
    
    row<-1:(dim(Stime)[1])
    col<-1:(dim(Stime)[1])
    val<-rep(1,(dim(Stime)[1]))
    Itime<-sparseMatrix(i=row,j=col,x=val)
    
    row<-1:(dim(Sspace)[1])
    col<-1:(dim(Sspace)[1])
    val<-rep(1,(dim(Sspace)[1]))
    Ispace<-sparseMatrix(i=row,j=col,x=val)
    
    # S is created using kronaker product
    S=LambdaS*kronecker(Sspace,Itime) + LambdaT*kronecker(Ispace,Stime)
    return(S)
}
