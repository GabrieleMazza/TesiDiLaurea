...
Popolazione<-read.table("Popolazione.txt",header=T)
names(Popolazione)
...
for (yearindex in 1:length(Years))
{
    ...
    ComuniLon<-NULL
    PLnorm<-NULL
    #Coefficiente di normalizzazione della popolazione
    norm<-1000
    attach(PostiLetto)
    for(i in 1:length(CodiciYear))
    {
        ...
        #La popolazione ogni norm abitanti
        tmp<-get(name)[Coord$Codice==CodiciYear[i]]/Popolazione$Popolazione[Popolazione$Anno==year & Popolazione$Codice==CodiciYear[i]]*norm
        PLnorm<-c(PLnorm,tmp)
    }
    detach(PostiLetto)
    ...
    X = matrix(1,nrow=length(PLnorm),ncol=1)
    X[,1] = PLnorm
