...
Create.Bspline.Time.Basis = function(TimePoints, TimeOrder, DerivativeOrder=1, PlotIt=FALSE, NumBasis=NA) 
{
    ...
    TimeBasisObj<-list(BasisObj=BasisObj,TimePoints=TimePoints,TimeOrder=TimeOrder,DerivativeOrder=DerivativeOrder)   
...
MakeS = function(SpaceBasisObj,TimeBasisObj,LambdaS,LambdaT)
{
    # Check arguments
    # Space
    if (class(SpaceBasisObj)!="basisfd")
    ...
    S=LambdaS*kronecker(Sspace,diag(dim(Stime)[1])) + LambdaT*kronecker(diag(dim(Sspace)[1]),Stime)
...
FixedTimePlot=function(t,SolutionObj,Nx=100,Ny=100)
{
    if (class(SolutionObj)!="SolutionObj")
    {  
        stop('Not valid SolutionObj in FixedPointPlot')
    ...
    xmin = min(SolutionObj$SpaceBasisObj$params$Points[,1])
    xmax = max(SolutionObj$SpaceBasisObj$params$Points[,1])
    X    = matrix(seq(xmin, xmax, len=Nx),ncol=1)
    
    ymin = min(SolutionObj$SpaceBasisObj$params$Points[,2])
    ymax = max(SolutionObj$SpaceBasisObj$params$Points[,2])
...
ReadSolutionObj = function(SpaceBasisObj,TimeBasisObj,FileName)
{
    ...
    if((dim(C)[1])!=SpaceBasisObj$nbasis*TimeBasisObj$BasisObj$nbasis)
        stop("Dimensions of C not correct")
...
ST.GCV = function(Data,SpaceBasisObj,TimeBasisObj,LogS,LogT)
{  
    # Check arguments
    # Space
    if (class(SpaceBasisObj)!="basisfd")
    ...
    GCVMatrix<-matrix(nrow=length(LogS),ncol=length(LogT))
    
    # Effective Degrees of Freedom Matrix
    EDFMatrix<-matrix(data=0,nrow=length(LogS),ncol=length(LogT))
    
    # Sigma^2 Hat Matrix
    Sigma2HatMatrix<-matrix(data=0,nrow=length(LogS),ncol=length(LogT))
    ...
    for(i in 1:length(LogS))
    {
        ...
        for(j in 1:length(LogT))
        {
            ...
            {
                Best<-c(LogS[i],LogT[j])
                BestGCV=GCVMatrix[i,j]
            }
        }
    }
    
    # Voglio aggiungere i nomi...
    NameS<-NULL
    NameT<-NULL
    for(i in 1:length(LogS))
    {
        NameS<-c(NameS,paste("S ",round(LogS[i],2),sep=""))
    }
    for(i in 1:length(LogT))
    {
        NameT<-c(NameT,paste("T ",round(LogT[i],2),sep=""))
    }
    ...
    Result<-list(Best=Best,GCVMatrix=GCVMatrix,EDFMatrix=EDFMatrix,Sigma2HatMatrix=Sigma2HatMatrix)
    
    return(Result)
}

ST.GCV.Covar = function(Data,DesMat,SpaceBasisObj,TimeBasisObj,LogS,LogT)
{  
    # Check arguments
    # Space
    if (class(SpaceBasisObj)!="basisfd")
    ...
    for(i in 1:length(LogS))
    {
        ...
        for(j in 1:length(LogT))
        {
            ...
            if((dim(HSm)[2])!=(dim(HCovar)[2]))
                stop("Something wrong with dimensions")
            # Effective Degrees of Freedom
            for(k in 1:(dim(HSm)[1]))
            {
...
ST.Smooth.Covar = function(Data,DesMat,SpaceBasisObj,TimeBasisObj,LambdaS,LambdaT)
{
    # SMOOTH.ST.FD Compute a solution for a Spatial-Time Spline problem 
    #
    # Arguments:
