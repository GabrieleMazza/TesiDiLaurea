...
basisobj = create.FEM.basis(p, e, t)
...
Meusefd = fd(numeric(nNodes),basisobj)
...
plot.FEM(ZincMeusefd$newfdobj)


#  evaluate the solution on the points

MeuseDataFitCovar = eval.FEM.fd(p[,1], p[,2], ZincMeusefdCovar$newfdobj) +  desmat %*% betahat



#  evaluate the solution on the points NO Covar

MeuseDataFit = eval.FEM.fd(p[,1], p[,2], ZincMeusefd$newfdobj)
...
windows()
plot.FEM(ZincResMeusefdCovar$newfdobj)
...
windows()
plot.FEM(ZincResMeusefd$newfdobj)
#title('\fontsize{16} Residuals for log10 of zinc concentration')
    ...
    for (idrop in 1:np)
     {
        ...
        fhati = Meusefdobji$newfdobj$coef
        betahati= ( solve( t(desmati) %*% desmati ) ) %*% t(desmati) %*% (datai[,2]-fhati[1:(np-1),])
        MeuseDataFiti = eval.FEM.fd(p[idrop,1], p[idrop,2], Meusefdobji$newfdobj) + desmat[idrop,] %*% betahati
    ...
    for (idrop in 1:np)
     {
        ...
        MeuseDataFiti = eval.FEM.fd(p[idrop,1], p[idrop,2], Meusefdobji$newfdobj)
        ressave[idrop] = data[idrop,2] - MeuseDataFiti
     }
...
#  Using the full Meuse bank area

MeuseTri_constr = as.matrix(read.table('MeuseTri_constr.txt', header=F))

#  define the vertices as points where there are data

p_constr = as.matrix(read.table('pMeuse_constr.txt', header=F))


p_constr_3col = p_constr
p_constr_2col = p_constr[,2:3]

#  The triangle index matrix

t_constr = MeuseTri_constr
nt_constr = dim(t_constr)[[1]]
...
polygon(c(p_constr_2col[MeuseTri_constr[1,1],1],p_constr_2col[MeuseTri_constr[1,2],1],p_constr_2col[MeuseTri_constr[1,3],1]),c(p_constr_2col[MeuseTri_constr[1,1],2],p_constr_2col[MeuseTri_constr[1,2],2],p_constr_2col[MeuseTri_constr[1,3],2]))
for (ne in 1:nt_constr)
{polygon(c(p_constr_2col[MeuseTri_constr[ne,1],1],p_constr_2col[MeuseTri_constr[ne,2],1],p_constr_2col[MeuseTri_constr[ne,3],1]),c(p_constr_2col[MeuseTri_constr[ne,1],2],p_constr_2col[MeuseTri_constr[ne,2],2],p_constr_2col[MeuseTri_constr[ne,3],2]))
}

print(paste("Number of edited triangles = ",nt_constr))
...
basisobj_constr = create.FEM.basis(p_constr_2col, e, t_constr)


#  set up the nodes, consisting of vertices plus edge midpoints
mknodes_constr= makenodes(p_constr_2col,t_constr)
nodes_constr=mknodes_constr$nodes
nodesmesh_constr=mknodes_constr$nodemesh
nNodes_constr = dim(nodes_constr)[[1]]
...
Meusefd_constr = fd(numeric(nNodes_constr),basisobj_constr)
...
lambdaCovar = 10^(3)
ZincMeusefdCovar_constr = smooth.FEM.fd.Covar(data,desmat,Meusefd_constr,lambdaCovar)



fhat_constr=ZincMeusefdCovar_constr$newfdobj$coef
betahat_constr= ( solve( t(desmat) %*% desmat ) ) %*% t(desmat) %*% (data[,2]-fhat_constr[1:np,])


#  plot smoothing surface computed using covariates
windows()
plot.FEM(ZincMeusefdCovar_constr$newfdobj)
...
MeuseDataFitCovar_constr = eval.FEM.fd(p[,1], p[,2], ZincMeusefdCovar_constr$newfdobj) +  desmat %*% betahat_constr



#  compute the residuals

MeuseResCovar_constr = cbind(data[,1], data[,2] - MeuseDataFitCovar_constr)
min(MeuseResCovar_constr[,2])
max(MeuseResCovar_constr[,2])
...
# ATTENZIONE: with what value of lambda?
lambda = 10^(3)
ZincResMeusefdCovar_constr = smooth.FEM.fd(MeuseResCovar_constr,Meusefd_constr,lambda)

windows()
plot.FEM(ZincResMeusefdCovar_constr$newfdobj)
...
dvec.FES_constr = numeric(np*(np-1)/2)
rvec.FES_constr = dvec.FES_constr
...
for (i in 2:np)
 {   
   for (j in 1:(i-1))
     {  m = m + 1
        dvec.FES_constr[m] = sum((p[i,]-p[j,])^2)
        rvec.FES_constr[m] = MeuseResCovar_constr[i,2] * MeuseResCovar_constr[j,2]
     }
 }


windows()
plot(log10(dvec.FES_constr), rvec.FES_constr,xlab="log_{10} Squared distance")
abline(h=0,col=2,lwd=2,lty="dashed")
...
nloglam = length(loglam)
SEsaveCovar_constr = numeric(nloglam)
for (ilam in 1:nloglam)
 {
    print(c(ilam,loglam[ilam]))
    lami = 10^loglam[ilam]
    ressave_constr = numeric(np)
    for (idrop in 1:np)
     {
        datai = data[-idrop,]
        desmati = desmat[-idrop,]
        Meusefdobji   = smooth.FEM.fd.Covar(datai,desmati,Meusefd_constr,lami)
        fhati = Meusefdobji$newfdobj$coef
        betahati= ( solve( t(desmati) %*% desmati ) ) %*% t(desmati) %*% (datai[,2]-fhati[1:(np-1),])
        MeuseDataFiti = eval.FEM.fd(p[idrop,1], p[idrop,2], Meusefdobji$newfdobj) + desmat[idrop,] %*% betahati
        ressave_constr[idrop] = data[idrop,2] - MeuseDataFiti
     }
    SEsaveCovar_constr[ilam] = sqrt(mean(ressave_constr^2))
}

print(rbind(loglam, SEsaveCovar_constr))

plot(loglam,SEsaveCovar_constr)
points(loglam,SEsaveCovar_constr,type="l")
...
##
indtri   = matrix(1:nt_constr,ncol=1)

xmin = min(p_constr_2col[,1])
xmax = max(p_constr_2col[,1])
nx   = 101
X    = matrix(seq(xmin, xmax, len=nx),ncol=1)
 
ymin = min(p_constr_2col[,2])
ymax = max(p_constr_2col[,2])
ny   = 101
...
evalmat = eval.FEM.fd(Xvec, Yvec, ZincMeusefdCovar_constr$newfdobj)
...
image(X,Y,evalmati,main=paste(c("Finite Elements splines,  betahat= " , betahat_constr[,1])),ylim=c(min(meuse.grid[,2]),max(meuse.grid[,2])),xlim=c(min(meuse.grid[,1]),max(meuse.grid[,1])),zlim=zlimits,xlab="",ylab="")
...
par(mfrow=c(2,2))
ylimits=c(min(c(rvec.Tps,rvec.FES_constr,rvec.Krig)),max(c(rvec.Tps,rvec.FES_constr,rvec.Krig)))
...
plot(log10(dvec.FES_constr), rvec.FES_constr,xlab="log_{10} Squared distance",main="Finite Elements splines",ylab="",ylim=ylimits)
abline(h=0,col=2,lwd=2,lty="dashed")
