...
Create.Bspline.Time.Basis = function(TimePoints, TimeOrder, PlotIt=FALSE)
{
    ...
    # Create the basis
    BasisObj = create.bspline.basis(c(min(TimePoints),max(TimePoints)), length(TimePoints), TimeOrder)
...
smooth.ST.fd = function(Data,SpaceBasisObj,TimeBasisObj,lambdaS,lambdaT)
{
    ...
    {
        C<-cbind(Sol[((j-1)*Spacenbasis+1):j*Spacenbasis,])
    }
...
MakeS = function(SpaceBasisObj,TimeBasisObj,LambdaS,LambdaT,DerivativeOrder=1)
{
    ...
    FunctionalPar = fdPar(fdobj=TimeBasisObj$BasisObj, Lfdobj=DerivativeOrder, lambda=LambdaT)
    
    Stime=smooth.basis(mean(TimeBasisObj$TimePoints), 1, FunctionalPar)$penmat
