...
L'uso di valori pro-capite per rifiuti e posti letto consente di replicare il dato del comune anche su altri punti in cui risulta necessario. Ad esempio le isole di Murano, Lido di Venezia e Pellestrina non sono sedi di comune, ma si riferiscono a Venezia. Quindi il dato di Venezia è stato replicato in queste isole ad ogni anno, per avere un valore di riferimento in quanto zone distaccate. Come si può notare in figura \ref{fig:Ven_rgm_zoom} anche nell'isola di Venezia è stato duplicato il dato, per avere una triangolazione  senza troppi triangoli composti solo da punti di frontiera in una zona di particolare rilevanza.
...
\begin{figure}[t]
	...
	\subfigure[S. Michele al Tagliamento]
   {
	...
	\caption{Stima della produzione dei rifiuti pro-capite in alcuni comuni.}
	\label{fig:Ven_tempo}
\end{figure}
...
\centering
	...
	\noalign{\hrule height 1.2pt}
	\end{tabular}
\caption{Analisi di $\mathrm{GCV}(\underline \lambda)$ per la provincia di Venezia, caso con covariata}
\label{tab:Vencovar}
\end{table}
...
\caption{Stima della funzione spazio-temporale della produzione dei rifiuti con covariata nella provincia di Venezia a tempi fissati: dal 1997 al 2011.}
...
\begin{figure}[t]
	...
	\subfigure[S. Michele al Tagliamento]
   {
	...
	\caption{Stima della produzione dei rifiuti pro-capite con covariata in alcuni comuni.}
	\label{fig:Ven_tempo}
\end{figure}
\newpage
