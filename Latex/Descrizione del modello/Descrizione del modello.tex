...
Sarebbe possibile trattare i dati come provenienti da funzioni tempo-varianti in ognuno dei punti spaziali considerati, cioè che le osservazioni $\{ z_{ij};i=1, ... , n; j=1, ... , m \}$ provengano da una funzione $z_i(t)$ valutata all'istante $t_j$. Questa marginalizzazione può analogamente essere eseguita in spazio, supponendo che per ogni istante di tempo esista una funzione variante in spazio definita su $\Omega$ che genera i dati. Tuttavia in questa analisi non si studiano questi modelli marginali, ma si suppone che le osservazioni $\{\underline z_{ij}; i=1, ... , n; j=1, ... , m\}$ siano generate da un campo campo spazio-temporale con  l'aggiunta di rumore:
\begin{equation}
\label{eq:modellobase}
z_{ij}=f(\underline p_i,t_j)+\varepsilon_{ij}\ \ \ \ i = 1,...,n\ \ j=1,...,m \ \ ,
\end{equation}
dove $\{\varepsilon_{ij}; i = 1,...,n; j=1,...m\}$ sono residui indipendenti identicamente distribuiti di media nulla e varianza $\sigma^2$. L'obiettivo del modello STR-PDE sarà la stima della funzione $f(\underline p,t)$ dalle $nm$ osservazioni a disposizione, minimizzando un funzionale $J(f(\underline p,t))$ con la somma degli scarti quadratici tra le osservazioni e valori stimati, e termini di penalizzazione per la regolarità della funzione separati in spazio e tempo.
...
Dall'analisi dei lavori analoghi già disponibili per modelli simili, anche se solo spaziali, si può dedurre che non è possibile dare una stima della funzione se non risulta espressa in una espansione di opportune funzioni di base. Infatti l'infinita possibilità di variazione di una funzione in un qualsiasi spazio funzionale non renderebbe possibile una stima computazionale del risultato. L'approccio scelto per la costruzione di $f(\underline p,t)$ si basa sulla generalizzazione delle espressioni in funzione di base dei casi puramente spaziali o temporali.

Siano $\{ \varphi_k(t);k=1, ... , M \}$  un insieme di $M$ funzioni di base definite sull'intervallo temporale $[T_1,T_2]$ e $\{ \psi_l(\underline p);l=1, ... , N \}$ in insieme di $N$ basi temporali definite sul dominio spaziale $\Omega$. Con le combinazioni lineari
$$
...
\end{equation}
Si ha quindi che $\{ a_k(\underline p);k=1, ... , M \}$ sono i coefficienti spazio-varianti dell'espansione in basi di tempo e $\{ b_l(t);l=1, ... , N \}$ sono i coefficienti tempo-varianti dell'espansione in basi spaziali. In questa costruzione la funzione $f(\underline p,t)$ possiede entrambe queste rappresentazioni.

Sono state ricavate due espressioni equivalenti per $f$, ma come si potrà notare in seguito questo è coerente, e sarà ricavata un'espressione più specifica. Per il momento occorre soltanto aggiungere condizioni per i coefficienti appena introdotti, che saranno necessarie per la costruzione del funzionale di penalizzazione $J(f(\underline p,t))$:
$$
a_k(\underline p) \in H_{n_0}^2 \qquad \forall k=1, ... , M
$$
e
$$
b_l(t) \in H^2 \qquad \forall l=1, ... , N \ ,
$$
dove $H^2_{0}(\Omega) = \{h \in H^2(\Omega) | \partial _{\nu}h=0 \mbox{ su } \partial \Omega\}$, spazio incluso in $H_2(\Omega)$ che garantisce alle funzioni $a_k(\underline p)$ di avere condizioni di Neumann alla frontiera.
...
Per una funzione spazio-variante sono disponibili più alternative per penalizzare la regolarità, ma in questo caso si considera la radice quadrata la norma $L^2$ del laplaciano. La stessa scelta è stata fatta in altre pubblicazioni come \cite{art:ramsay}, \cite{art:sangalli} e \cite{art:wood}. Quindi se $g(\underline p): \Omega \mapsto \mathbb{R}$ è una funzione spazio-variante, allora si può definire la penalizzazione in spazio come
$$
J_S\left(g(\underline p)\right)=\int_{\Omega} \Bigl( \Delta  g(\underline p  ) \Bigr)^2 d \underline p \ .
$$
Analogamente in tempo si avrà la penalizzazione della radice quadrata della norma $L^2$ della derivata seconda. Se $h(t): [T_1,T2] \mapsto \mathbb{R}$, allora si avrà
$$
...
\end{multline}
dove $\lambda_S>0$ e $\lambda_T>0$ sono i parametri di smoothing che stabiliscono il peso della penalizzazione della regolarità della funzione rispettivamente in spazio e tempo. 
...
Se si applica la forma per $f(\underline p,t)$ dell'eq. \ref{eq:f_temp} nel termine di penalizzazione del laplaciano in spazio, allora si può ritrovare:
\begin{equation} 
...
Allo stesso modo, se si sostituisce la forma per $f(\underline p,t)$ dell'eq. \ref{eq:f_space} nella penalizzazione ideale dell'eq. \ref{eq:Jfunc_cont} si ottiene:
\begin{equation} 
...
\subsection*{Discretizzazione dei termini di penalizzazione di $J_{\underline \lambda }(f(\underline p,t)))$}
...
Il primo caso da analizzare è l'integrale del laplaciano dei coefficioenti spazio-varianti dell'eq. \ref{eq:f_temp} in modo analogo a quanto fatto in \cite{art:sangalli}. Fissato $k$, l'integrale
$$
\int_{\Omega} \Bigl( \Delta(  a_k(\underline p)  ) \Bigr)^2 d \underline p
$$
può essere semplificando introducendo la funzione $g_k(\underline p)$ come segue:
\begin{equation}
\label{eq:ps1}
...
\end{equation}
Grazie alla la formula di Green ed alle condizioni condizioni di Neumann per $a_k(\underline p)$, si possono semplificare gli integrali come segue:
$$
...
$$
Per poter calcolare analiticamente questi integrali è necessario introdurre l'uso delle basi spaziali $\{ \psi_l(\underline p);l=1, ... , N \}$ per le funzioni $a_k$, $g_k$ e $v$. Siano quindi:
$$
...
\end{equation}
Grazie all'uso delle funzioni di base e a ciò che è stato ottenuto dall'applicazione della formula di Green, le relazioni \ref{eq:ps1} e \ref{eq:ps2} diventano:
$$
...
alla valutazione di una forma quadratica che non cambia con le funzioni $a_k$. Ma si ha anche un'altra conseguenza: per l'equivalenza ipotizzata tra le espressioni di $f(\underline{p},t)$ in \ref{eq:f_temp} e \ref{eq:f_space}, allora è necessario che i coefficienti tempo-varianti dell'espansione in basi spaziali assumano la seguente forma:
$$
b_l(t)=\sum_{k=1}^M c_{lk}\varphi_k(t) \ .
$$
Si ritrova quindi anche in questo caso l'espansione in funzioni di base.

Non resta altro che discretizzare anche $\int_{T_1}^{T_2} \Bigl( \frac{\partial^2   b_l(t)   }{\partial t ^2} \Bigr)^2 dt$. Ma dopo aver introdotto l'uso delle funzioni di base, allora se si definisce 
 $$ P_T = \begin{bmatrix}
...
\end{bmatrix} \ .
$$
Allora, se si definisce la matrice $B$ tramite il prodotto di Kronecker
$$ B = \Psi \otimes \Phi \ ,$$
si può facilmente dire che
\begin{equation}
\begin{bmatrix}
f(\underline p_1,t_1)  \\
\vdots\\
...
\end{equation}
e per ricavare la stima della funzione $f(\underline{p},t)$ sarà sufficiente ricavare il vettore dei coefficienti $c_kl$ risolvendo il problema di minimo:
$$
\hat{\underline{c}}=\argmin_{c \in \mathbb{R}^{NM}} J_{\underline \lambda }(\underline c) \ .
$$
Grazie alla formulazione ricavata in \ref{eq:Jmatr}, allora basta derivare per ottenere la soluzione al problema di stima. Grazie alla simmetria di $P$, si ha:
$$
...
Il problema di stima è stato risolto, e la soluzione si ricava solo dalla risoluzione di un sistema lineare, seppur di grandi dimensioni (la matrice $(B^T B + P)$ ha dimensioni $NM \times NM$, e già dagli esempi si potranno notare dimensioni elevate).
...
$$ \hat f = \argmin_{\underline c \in \mathbb{R}^{NM}} J_{\underline \lambda }(f) .$$


, $\{ c_{lk};l=1, ... , N;k=1, ... , M \}$ sono coefficienti dell'espansione della funzione in basi separabili spazio-temporali. Le espressioni di $f(\underline p,t)$ riportate spiegano che il campo spazio-temporale è il risultato della generalizzazione dei modelli marginali rispettivamente in tempo e in spazio. Perciò la formulazione nell'eq. \ref{eq:basisexp} deriva dall'introduzione della possibilità, per i coefficienti dell'espressione in basi di tempo in \ref{eq:f_temp}, di poter variare in spazio o analogamente, per i coefficienti dell'espressione in basi spaziali di \ref{eq:f_space}, di poter variare in tempo. Ne consegue che anche essi siano esprimibili con le rispettive basi:
$$
a_k(\underline p)=\sum_{l=0}^N c_{lk}\psi_l(\underline p)
$$
La formulazione finale della funzione $f(\underline p,t)$ è quindi la combinazione lineare di tutti i possibili prodotti incrociati tra i due insiemi delle funzioni di base (eq. \ref{basisexp}), ma concettualmente proviene dalla generalizzazione dei modelli marginali.  

Il fondamento della modellizzazione del campo spazio-temporale sui modelli marginali in spazio e tempo introduce il concetto di separabilità più volte presente in questa formulazione. Come si potrà notare anche in seguito, anche i termini di penalizzazione saranno derivati dall'estensione di termini puramente spaziali o temporali.
